...
import Car from "../../assets/fullCar.png";
...
export const Cars = () => {
    const stepsData = [
        ...
        {
            title: 'Выбор машины', description: <>
        ...
        <div className={style.header} id="about">
            <div className={style.header__left}>
                <div className={style.header__left__benchmark}>
                    <span className={style.lock}><img src={lock} />Безопасные сделки по автомобилям</span>
                    <span>·</span>
                    <span>Гарантия XPORT Escrow </span>
                </div>

                <h1 className={style.header__left__title}>Ваш <span>персональный байер</span> по покупке/продаже автомобилей</h1>
                <p className={style.header__left__description}>Автомобили под защитой. Покупка и продажа без рисков и переплат. Мы гарантируем безопасную сделку через XPORT Escrow</p>
                ...
                <div className={style.header__left__buttons}>
                    <a className={style.greenButton} onClick={(e) => handleAnchorClick('cars_buyers', e)}><p>Начать сделку </p> <img className={style.arrow} src={arr} /></a>
                </div>
            </div>
            <div className={`${style.header__right} ${style.car}`}>
                <img src={Car} />
            </div>
        </div>
        <HowItWorks stepsData={stepsData} subtitle={'Найдите машину своей мечты с полной уверенностью в безопасности сделки.'} />
        <div className={`${style.buyers} ${style.cars}`} id='cars_buyers'>
            <TrustedBuyersList market={'Автомобили'} />
        </div>
    </div>)
}